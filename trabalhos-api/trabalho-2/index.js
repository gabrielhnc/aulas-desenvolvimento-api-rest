...
app.post('/api/jogos', (req, res) => {
    ...
    if (typeof titulo !== 'string' || typeof desenvolvedora !== 'string' || typeof genero !== 'string' || typeof plataforma !== 'string') {
        return res.status(400).json({
            erro: "Titulo, desenvolvedora, genero e plataforma devem ser texto"
        });
    }
...
// GET JOGOS (filtros, ordenação e paginação)
app.get('/api/jogos', (req, res) => {
    ...
    if (genero) {
        resultado = resultado.filter(j => j.genero === genero);
    }
    ...
    if (ano_min) {
        resultado = resultado.filter(j => j.ano >= parseInt(ano_min));
    }

    if (ano_max) {
        resultado = resultado.filter(j => j.ano <= parseInt(ano_max));
    }

    if (ordem) {
        resultado.sort((a, b) => {
            if (ordem === 'nota') return direcao === 'desc' ? b.nota - a.nota : a.nota - b.nota;
            if (ordem === 'ano') return direcao === 'desc' ? b.ano - a.ano : a.ano - b.ano;
            if (ordem === 'titulo') return direcao === 'desc'
